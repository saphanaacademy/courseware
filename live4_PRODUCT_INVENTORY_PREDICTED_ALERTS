...
(
	SELECT 
		...
		ROW_NUMBER() OVER (
	 		PARTITION BY WS.WERKS, IA.MATNR 
	 		ORDER BY 
	 			NEW ST_Point(WS.STR_LONGITUDE, WS.STR_LATITUDE).ST_Distance(NEW ST_Point(WST.STR_LONGITUDE_T, WST.STR_LATITUDE_T)) ASC, 
	 			IA.INVENTORY_PREDICTED_SPARE DESC
		) AS RANK
 	FROM LIVE4.LIVE4.T001W_WS WS
	LEFT JOIN
	 (
		SELECT
	 		PI.WERKS,
		...
		FROM LIVE4.PRODUCT_INVENTORY PI 
		INNER JOIN LIVE4.MAKT MA ON (MA.MATNR = PI.MATNR) 
		INNER JOIN LIVE4.PAL_P_DATA PD ON (PD.WERKS = PI.WERKS AND PD.MATNR = PI.MATNR) 
		INNER JOIN LIVE4.PAL_FITTED PF ON (PF.ID = PD.ID)
		WHERE (ROUND(PI.INVENTORY) - ROUND(PF.FITTED)) < 0
	 ) IA
	...
	WHERE IA.INVENTORY_PREDICTED_SPARE + WST.INVENTORY_PREDICTED_SPARE_T > 0
